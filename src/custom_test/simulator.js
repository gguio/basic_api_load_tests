...
const config = {
    baseURL: 'https://gguio.org/api/test', 
    users: 500, // кол-во виртуальных пользователей
    duration: 600, // время теста
    delayRange: [1000, 5000], // диапазон задержек между запросами в мс
};
...
async function runSimulation() {
    ...
    setTimeout(() => {
        ...
        const maxResTime = responseTimes.sort((a, b) => a > b)[0]
        ...
        console.log(`Maximum Response Time: ${maxResTime.toFixed(2)}ms`);
        console.log(`Bandwidth: ${bandwidth.toFixed(2)} requests/sec`);
        console.log(`Error Details:`, errorDetails);
    }, config.duration * 1000 + config.delayRange[1]);
}
